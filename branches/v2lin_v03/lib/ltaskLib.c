...
static int translate_priority(int v2pthread_priority, int sched_policy, int *errp)
{
	...
	if (v2pthread_priority < min_priority)
	{
		pthread_priority = min_priority;
		printf ("\n\r translate_priority - too low %d\n\r", v2pthread_priority);
		*errp = S_taskLib_ILLEGAL_PRIORITY;
	}
	else if (v2pthread_priority > max_priority)
	{
		pthread_priority = max_priority;
		printf ("\n\r translate_priority - too high %d\n\r", v2pthread_priority);
		*errp = S_taskLib_ILLEGAL_PRIORITY;
	}
	else
	{
				...
				int arg6, int arg7, int arg8, int arg9, int arg10)
{
    ...
    /* ----- set the real stack size -------*/
    if (stksize < MIN_V2LIN_TASK_SIZE)
        stksize = MIN_V2LIN_TASK_SIZE;
    /*--------------------------------------------*/
    pthread_attr_setstacksize(&task->attr, stksize);
    /*--------------------------------------------*/
...
exit:
    ...
    //----- Events for task 'receive' ------*
    pthread_mutex_init(&task->tevent_lock, NULL);
    pthread_cond_init(&task->t_eventable, NULL);
    // First task control block in list of tasks waiting to receive a message from queue
	task->event_first_susp = NULL;
    task->signaled_events  = 0;
    //--------------------------------------------*
...
STATUS taskActivate(int tid)
{
	...
	if (task ) {
		...
		if (task->state == DEAD) {
			...
			if (pthread_create(&task->pthrid, &task->attr, (start_routine) task_wrapper, task) != 0) {
				...
				int policy;
				pthread_attr_getschedpolicy(&task->attr, &policy);
				v2pt_set_realtime_prio(task->pthrid, policy, task->prv_priority.sched_priority);
			}
		} else {
			/*
    ...
    {
        /* set the name for a newly created thread */
    	if (pthread_setname_np(task->pthrid, task->taskname) != 0)
    	{
    		char perror_str[50] = { 0 };
    		snprintf(perror_str, 50, "pthread_setname_np: %s", task->taskname);
    		perror(perror_str);
    	}
    }
	return error;
			  ...
			  int arg5, int arg6, int arg7, int arg8, int arg9, int arg10)
{
	...
	if (error != OK) {
		TRACEF("taskInit failed err %d", error);
		ts_free((void *) task);
		goto exit;
	}
  ...
  exit:
	...
	if (error != OK) {
		TRACEF("taskActivate tid %d failed err %d", my_tid, error);
...
void display_task(FILE * out, unsigned long tid)	// OLD
{
	TRACEF();
	int policy;
	int detachstate;
...
void taskShow(task_t *t, V2Lin_PrintCb_T printFunc_p, void *cookie)
{
	//TRACEF("%x", t);
	int policy;
	int detachstate;
	...
	printFunc_p(cookie, "%#10x %10x %10s ", t->taskid, t, t->taskname);
	printFunc_p(cookie, "th:%x ", t->pthrid);
	printFunc_p(cookie, "s:%#x ", t->state);
	
	if ( t->state & PEND  ) 
		printFunc_p(cookie, "P ");
	
	if ( t->state & DELAY  ) 
		printFunc_p(cookie, "D " );
	
	printFunc_p(cookie, "w:%x ", t->waiting);
	
	if (t->waiting && t->waiting->current_owner)  {
		printFunc_p(cookie, "%s ", t->waiting->current_owner->taskname);
	}
	
	printFunc_p(cookie, "w:%x ", t->waiting_m);
	printFunc_p(cookie, "s:%x ", t->first_susp);
	//printFunc_p(cookie, " Thread ID: %lx  Vxworks priority: %d", t->pthrid, t->vxw_priority);

	pthread_attr_getschedpolicy(&t->attr, &policy);
	//pthread_getschedparam(task->pthrid, &sched_policy2, &schedparam2);
	switch (policy) {
	case SCHED_FIFO:
		printFunc_p(cookie, "SCHED_FIFO ");
		break;
	case SCHED_RR:
		printFunc_p(cookie, "SCHED_RR ");
		break;
	case SCHED_OTHER:
		printFunc_p(cookie, "SCHED_OTHER ");
		break;
	default:
		printFunc_p(cookie, "schedpolicy: %d ", policy);
	}
	
	pthread_attr_getschedparam(&t->attr, &schedparam);
	
	printFunc_p(cookie, " pri %d ", schedparam.sched_priority);
	printFunc_p(cookie, " prv_pri %d ", t->prv_priority.sched_priority);
	
	pthread_attr_getdetachstate(&t->attr, &detachstate);
	
	//printFunc_p(cookie, " detachstate %d ", detachstate);
	printFunc_p(cookie, "\r\n");
}
...
int taskList(V2Lin_PrintCb_T printFunc_p, void *cookie, int mem)
{
	...
	TRACEF();
	printFunc_p(cookie, "\r\n");
	printFunc_p(cookie, "locker=%x\r\n", locker);
	printFunc_p(cookie, "taskLock_level=%x\r\n", taskLock_level);
	task_t *t = task_list;
	while (t) {
		taskShow(t, printFunc_p, cookie);
		if (mem) {
			int *w;
...
/**************************************************************************/
/** Event vxWorks library, the implementation based on pthread_cond_wait **/
/**************************************************************************/
STATUS eventReceive(unsigned int events, unsigned char flags, int timeOut, unsigned int * events_r)
{
    ...
    pthread_cleanup_push((void (*)(void *)) pthread_mutex_unlock, &self_task->tevent_lock);
    pthread_mutex_lock(&self_task->tevent_lock);

    if (EVENTS_FETCH & flags)
    {
        *events_r = self_task->signaled_events;
        goto exit;
    }

  	// Add task for task to list of tasks waiting on queue
	link_susp_task(&self_task->event_first_susp, self_task);

    retcode = 0;

    if (timeOut == WAIT_FOREVER) {
        //  Infinite wait was specified... wait without timeout.
        while (  !( (flags & EVENTS_WAIT_ANY) && (self_task->signaled_events  & events)) && 
                 !(!(flags & EVENTS_WAIT_ANY) && ((self_task->signaled_events & events) == events)) ) {
            {
                retcode = pthread_cond_wait(&self_task->t_eventable, &self_task->tevent_lock);
            }
        }
    } else {
        long sec, usec;
         ...
         */
        sec = 0;
        usec = timeOut * V2PT_TICK * 1000 + 10;
        ...
        {
           if ( ( (flags & EVENTS_WAIT_ANY) && (self_task->signaled_events  & events)) || 
                (!(flags & EVENTS_WAIT_ANY) && ((self_task->signaled_events & events) == events)) )
               break;

           retcode = pthread_cond_timedwait(&self_task->t_eventable, &self_task->tevent_lock, &timeout);
        }
    }
    ...
    /*
      ...
      */
     unlink_susp_task(&self_task->event_first_susp, self_task);

     *events_r = ((EVENTS_RETURN_ALL & flags)? self_task->signaled_events: (self_task->signaled_events & events));
     self_task->signaled_events &= ~(events);

    if (retcode == ETIMEDOUT) 
    {
        error = ((timeOut == NO_WAIT)? S_eventLib_NOT_ALL_EVENTS: S_eventLib_TIMEOUT);
    }

exit:
      pthread_mutex_unlock(&self_task->tevent_lock);
      pthread_cleanup_pop(0);
...
STATUS eventSend(int taskId, unsigned long events)
{
    ...
    pthread_cleanup_push((void (*)(void *)) pthread_mutex_unlock, &t->tevent_lock);
    pthread_mutex_lock(&t->tevent_lock);

    // Set relevant events
    t->signaled_events |= (events);

    // Signal the condition variable for the event
    pthread_cond_broadcast(&t->t_eventable);

    pthread_mutex_unlock(&t->tevent_lock);
    pthread_cleanup_pop(0);
...
STATUS v2pt_set_realtime_prio(pthread_t thread, int policy, int priority)
{
    ...
    status = pthread_setschedparam(thread, policy, &params);
    
    if (status) {
    	perror("v2pt_set_realtime_prio: pthread_setschedparam");
    	return status;
    }
    /* 
     * Now verify the change
     */
    status = pthread_getschedparam(thread, &policy_check, &params);
    if (status) {
    	perror("v2pt_set_realtime_prio: pthread_getschedparam");
    	return status;
    }
 
#if 1 /* for debugging stage */
    if((policy_check != policy) || (params.sched_priority != priority)) {
        printf("\n\rv2pt_set_realtime_prio: \n\r\t\
failed to apply policy/prio for thread %lu!\n\r", thread);
    }
#endif
