...
//	DBG_TRACE("looking for watchdog @ %p\n", wdId);
...
STATUS wdStart(v2pt_wdog_t * wdId, int delay, void (*f) (int), int parm)
{
	...
	if (wdog_valid(wdId)) {
		 ...
		 ** 
		 ** checked: 10 msec works pretty well
		 */
		wdId->ticks_remaining = delay;
