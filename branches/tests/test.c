...
void v2lin_usrPrintBuffer(char *buffer, void *cookie)
{
	/* user implementation of the formatted output */
}

void v2lin_usrPrintWrapper(void *cookie, char *fmt, ...)
{
#if 0 /* user implementation of the formatted output */
    char buffer[512];
	va_list args;
	va_start(args, fmt);
	vsprintf(buffer, fmt, args);
	va_end(args);
	v2lin_usrPrintBuffer(buffer, cookie);
#endif
	printf(fmt);
}
...
void smaphores_create()
{
	...
	//semList(v2lin_usrPrintWrapper, NULL, 0);
  exit:;
}
...
{
	TRACEF();
	taskList(v2lin_usrPrintWrapper, NULL, 0);
	semList(v2lin_usrPrintWrapper, NULL, 0);
}
...
int test()
{
	...
	msgQList(v2lin_usrPrintWrapper, NULL, 1);
	enableRoundRobin();
...
void test_wait()
{
	...
	TRACEV("%i", status);
	TRACE_errno();
	CHK(3 == taskList(v2lin_usrPrintWrapper, NULL, 0));
	switch (errno) {
		case OK:
			puts("Test finished");
			break;
