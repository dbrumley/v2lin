...
void semShow(v2lsem_t *s, V2Lin_PrintCb_T printFunc_p, void *cookie, int mem)
{
	task_t *t;
	printFunc_p(cookie, "o:%x ", s->current_owner);
	if (s->current_owner) { 
		...
		if (!t) // task not found
			printFunc_p(cookie, "<deleted> ");
		else printFunc_p(cookie, "%s ", s->current_owner->taskname);
	}
	printFunc_p(cookie, "rl:%i ", s->recursion_level);
	printFunc_p(cookie, "tc:%i ", s->token_count);
	printFunc_p(cookie, "(");
	for (t = s->first_susp; t; t = t->nxt_susp) {
		printFunc_p(cookie, "%x ", t);
		printFunc_p(cookie, "%s ", t->taskname);
	}
	printFunc_p(cookie, ") ");
	printFunc_p(cookie, "%x ",s->send_type);

	if (mem) {
		int *i;
		for (i = (int*)s; i < (int*) (s + 1); i++) 
			printFunc_p(cookie, "%x ",*i);
	}
	printFunc_p(cookie, "\r\n");
}
...
int semList(V2Lin_PrintCb_T printFunc_p, void *cookie, int mem)
{
	...
	while (s) {
		c++;
		printFunc_p(cookie,"%i %x: ", c, s);
		semShow(s, printFunc_p, cookie, mem);
		s = s->nxt_sem;
	}
	return c;
}
...
static int sem_find_lock(v2lsem_t * sem)
{
	...
	for (current_smcb = sem_list; current_smcb != NULL; current_smcb = current_smcb->nxt_sem) {
		if (current_smcb == sem) {
			...
			/* do no lock a semaphore that is being deleted */
			if (sem->send_type & KILLD) {
...
v2lsem_t *new_sem(int count)
{
	...
	semaphore->delete_allowed = FALSE;
...
#if 0   /* TODO: decide if we want to handle this */
		errno = ENOSYS;
		return (NULL);
#endif
	}
...
/*****************************************************************************
** semOpen - creates a v2pthread mutual exclusion semaphore
** TODO: This is a temporary solution for a single Linux process
** 
*****************************************************************************/
v2lsem_t *semOpen(const char * name, int type, int initState,
...
STATUS semDelete(v2lsem_t * semaphore)
{
	...
	//  If no one is waiting on the semaphore, just set its type to KILLED.
	//  Otherwise, send signal and block while any tasks are still waiting.
	if (!semaphore->first_susp) {
		...
		do {
			 ...
			 */
			while (!semaphore->delete_allowed)
...
STATUS semFlush(v2lsem_t * semaphore)
{
	 ...
	 */
	if (semaphore->first_susp ) {
		// Lock mutex for semaphore flush completion
		pthread_cleanup_push((void (*)(void *)) pthread_mutex_unlock, &semaphore->smdel_lock);
		pthread_mutex_lock(&(semaphore->smdel_lock));
		...
		/*
		 **  Wait for all pended tasks to receive flush message.
		 **  The last task to receive the message will signal the
		 **  flush-complete condition variable.
		 */
		while (semaphore->first_susp != NULL)
			pthread_cond_wait(&(semaphore->smdel_cplt), &(semaphore->smdel_lock));
		pthread_cleanup_pop(1);
	} else {
		// no one's waiting - just let the mutex go
		pthread_mutex_unlock(&(semaphore->sem_lock));
	}
...
{
	int result;
	if (semaphore->send_type != SEND) {
		//  Semaphore has been killed or flushed... waiting is over.
		result = 0;
		*retcode = 0;
		goto exit;
	}
...
STATUS wait_for_token(v2lsem_t * semaphore, int max_wait, task_t * our_task)
{
	...
	if (max_wait == NO_WAIT) {
		...
		if (max_wait == WAIT_FOREVER) {
            ...
            {
				TRACEF("pthread_cond_timedwait {");
				if ( pthread_cond_wait(&semaphore->sem_send, &semaphore->sem_lock) != 0 )
                    break; 
				TRACEF("pthread_cond_timedwait }");
            } 
		} else {
	...
	if ((semaphore->send_type & KILLD) || (semaphore->send_type & FLUSH)) {
		...
		if ( ! semaphore->first_susp ) {
			pthread_cleanup_push((void (*)(void *)) pthread_mutex_unlock, &semaphore->smdel_lock);
			if (semaphore->send_type & FLUSH)
				semaphore->send_type = SEND;
			pthread_mutex_lock(&semaphore->smdel_lock);
			if (semaphore->send_type & KILLD)
				semaphore->delete_allowed = TRUE;
